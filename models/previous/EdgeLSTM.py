...
class EdgeLSTMTrain(nn.Module):
    def __init__(self, node_in_dim, node_hidden_dim, edge_in_dim, num_class, num_layers, num_lstm_layers, bidirectional, device, dropout=None):
        super(EdgeLSTMTrain, self).__init__()
...
class EdgeLSTMInfer(nn.Module):
    def __init__(self, node_in_dim, node_hidden_dim, edge_in_dim, num_class, num_layers, num_lstm_layers, bidirectional, device, dropout=None):
        super(EdgeLSTMInfer, self).__init__()
    ...
    def forward(self, nf):
        ...
        for i, (edge_layer, edge_out_layer, node_layer) in enumerate(zip(self.edge_layers, self.edge_out_layers, self.node_layers)):
            ...
            def message_func(edges):
                ...
                e = edges.data['e']    
                e_len = edges.data['e_len']    
                e_times = edges.data['e_times']  # time features

                e = torch.cat((e, e_times.unsqueeze(-1)), dim=-1) # use raw time features
                e = edge_layer(e, e_len)
