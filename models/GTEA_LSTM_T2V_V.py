...
class GTEALSTMT2V_VTrain(nn.Module):
    def __init__(self, node_in_dim, node_hidden_dim, edge_in_dim, time_hidden_dim, num_class, num_layers, num_lstm_layers, bidirectional, device, dropout=None):
        super(GTEALSTMT2V_VTrain, self).__init__()
        ...
        self.edge_attentiion_layers.append(LSTM(edge_in_dim + time_hidden_dim, node_hidden_dim, num_lstm_layers, device, bidirectional=bidirectional))
    ...
    def forward(self, nf):
        ...
        for i, (time_layer, edge_layer, edge_att_layer, attn_layer, edge_out_layer, node_layer) \
        in enumerate(zip(self.time_layers, self.edge_layers, self.edge_attentiion_layers, self.attn_layers, self.edge_out_layers, self.node_layers)):
            ...
            def message_func(edges):
                ...
                e_out = edge_layer(e, e_len)

                a = edge_att_layer(e, e_len)
...
class GTEALSTMT2V_VInfer(nn.Module):
    def __init__(self, node_in_dim, node_hidden_dim, edge_in_dim, time_hidden_dim, num_class, num_layers, num_lstm_layers, bidirectional, device, dropout=None):
        super(GTEALSTMT2V_VInfer, self).__init__()
