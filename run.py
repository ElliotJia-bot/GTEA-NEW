...
models = ['GCN', 'GraphSAGE', 'GAT', 'ECConv', 'GTEA-ST', 'EGNN']
test_time = 2
num_layers = [1, 2]
num_neighbors = [10]
node_hidden_dim = [64, 128, 256]
num_heads = [4]
                        ...
                        for d in node_hidden_dim:
                            for bs in batch_size:
                                    log_name = "{model}_{num_neighbors}_{node_hidden_dim}_{num_layers}_{num_heads}_{test_time}".format(
                                        model=m, num_neighbors=n, node_hidden_dim=d, num_layers=l, num_heads=1,
                                        test_time=t)
                                    print(log_name)
                                    os.system(
...
models = ['TGAT', 'GTEA-LSTM', "GTEA-TLSTM1", "GTEA-LSTM+T2V", "GTEA-Trans", "GTEA-Trans+T2V"]
leaning_rates = [1e-4]
                                    ...
                                    for h in num_heads:
                                        for lr in leaning_rates:
                                            ...
                                            os.system(
                                                ...
                                                "--num_layers {num_layers} --num_heads {num_heads} --num_cpu {num_cpu} "
                                                "--log_dir {log_dir} --log_name {log_name} --num_lstm_layers {num_lstm_layers} "
                                                "--batch_size {batch_size}  --lr {lr} --patience {patience} --time_hidden_dim 4".format(data_dir=data_dir, model=m, 
                                                                                                   gpu=gpu,epochs=epochs,num_neighbors=n,
